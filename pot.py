    ...
    def drawPoints(
        ...
        try:
            for p in range(0, pixel_height - pixel_center_y, step_im):
                ...
                cv.circle(image, (pixel_center_x, pixel_y), point_radius, (0, 0, 255), -1)
                cv.circle(image, (pixel_x1, pixel_y), point_radius, (255, 0, 0), -1)
                cv.circle(image, (pixel_x2, pixel_y), point_radius, (255, 0, 0), -1)
...
class PotElliptical(Pot):
    ...
    def __post_init__(self):
        self.coef1 = self.basis_diameter_cm
    ...
    def getRadiusAtHeight(self, y: int):
        return self.coef1 * sqrt(1 + self.coef2 * (y**2))


if __name__ == '__main__':
    image = np.zeros((400, 400, 3), dtype=np.uint8)
    pot = PotElliptical(30, 40)
